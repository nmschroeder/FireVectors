...
instrument <- user_inputs[[2]]
satname <- user_inputs[[3]]
...
# taskID <- 6212 # 4490 for North Start Fire (troubleshooting dAdt < 0 ); 6880 for Bolt Creek # 6212 for August Complex; 6347 for East Troublesome; 2075 for Zaca Fire (2007)
# instrument <- "VIIRS"
# satname <- "Aqua" # Use for computing speed (all VIIRS will be used for perims)
...
# Last updated August 26, 2025, 1:09 PM MDT
...
if (data_available == 1){
  ...
  save_fname <- paste0("active_fire_pixel_data_", tolower(instrument), "_", 
                       str_pad(taskID, width = 5, pad = "0"), ".geojson")
...
if (data_available == 1 & N_p > 1){
  ...
  # Initialize a vector for the start time of each grouped set of measurements
  temp_data <- perim_df %>% dplyr::filter(SATELLITE == satname)
